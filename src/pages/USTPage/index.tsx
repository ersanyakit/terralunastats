...
const USTPage: React.FunctionComponent<IPage> = props => {
    ...
    const fetchTotalAndCirculationSupply = async () => {
        await fetch(`https://api.coingecko.com/api/v3/coins/terrausd?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false`)
            .then(res => res.json())
            .then(res => {
                setTotalSupply(res.market_data.total_supply ? res.market_data.total_supply : res.market_data.circulating_supply);
    ...
    const fetchBurnedSupply = async () => {
        const data = await fetch(`https://api.terralunastats.com/api/transactions/ust/burned`)
            .then(res => res.json())
            .then(res => {
...
*/
    ...
    useEffect(()=>{
        document.title = props.title + " - Terra $UST Burn Machine";
    },[]);
    return(
        <div>
            ...
            <div className={"row"}>
                <div className={"col-12 d-flex flex-column align-items-center justify-content-center mb-5"}>
                    <h1 className={"text-yellow"}>Terra $UST Burn Address</h1>
                ...
                <div className="drive-info">
                    ...
                    <div className={"d-flex align-items-center w-100 text-center my-3"}>
                        <a className="twitter-share-button w-100"
                           target={"_blank"}
                           href={`https://twitter.com/intent/tweet?text=%23USTBurn%0ATotalSupply:${numberWithCommas(totalSupply)}%20$UST%0ACirculationSupply:${numberWithCommas(circulationSupply)}%20$UST %0ABurned:${numberWithCommas(totalBurnedSupply)} $UST%0AMarketcap:$${numberWithCommas(marketCap)}%0AVolume24h:$${numberWithCommas(volume24h)}%0AChange24h:${change24h}%0APrice:$${price}%0A%0Ahttps%3A%2F%2Fterralunastats.com`}
                           data-size="large">
